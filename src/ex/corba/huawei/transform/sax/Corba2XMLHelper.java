...
public class Corba2XMLHelper {
	...
	public List<HW_MSTPEndPoint_T> printMstpEndPoints(HW_MSTPEndPoint_T[] list)
		...
		for (HW_MSTPEndPoint_T endPoint : list) {
			if (endPoint.type == HW_MSTPEndPointType_T
					.from_int(HW_MSTPEndPointType_T._HW_MEPT_ETHTRUNK)) {
	...
	public Corba2XMLContainer getMstpEndPointParams(HW_MSTPEndPoint_T endPoint)
			throws ProcessingFailureException {
		Corba2XMLContainer container = new Corba2XMLContainer(
				Corba2XMLStructure.ENDPOINTS);
		container.setFieldValue(CorbaConstants.USER_LABEL_STR,
				endPoint.userLabel);
		container.setFieldValue(CorbaConstants.NATIVE_EMS_NAME_STR,
				endPoint.nativeEMSName);
		container.setFieldValue(CorbaConstants.OWNER_STR, endPoint.owner);
		container.setFieldValue(CorbaConstants.NAME_STR,
				handler.convertNameAndStringValueToString(endPoint.name));

		container.setFieldValue(CorbaConstants.TYPE_STR,
				String.valueOf(endPoint.type.value()));
		LayeredParameters_T[] transmissionParamsArray = endPoint.transmissionParams;

		if (transmissionParamsArray == null
				|| transmissionParamsArray.length == 0) {
		...
		container.setFieldValue(CorbaConstants.ADDITIONAL_INFO_STR, handler
				.convertNameAndStringValueToString(endPoint.additionalInfo));

		return container;
	}

	public void printEthServices(HW_EthService_T[] list) throws SAXException,
	...
	public Corba2XMLContainer getEthServiceParams(HW_EthService_T value)
			throws ProcessingFailureException {
		Corba2XMLContainer container = new Corba2XMLContainer(
				Corba2XMLStructure.ETHSERVICES);
		...
		container.setFieldValue(CorbaConstants.A_END_NE_STR, handler
				.getValueByName(value.aEndPoint.name,
						CorbaConstants.MANAGED_ELEMENT_STR));
		container.setFieldValue(CorbaConstants.A_END_PTP_STR, handler
				.getValueByName(value.aEndPoint.name, CorbaConstants.PTP_STR));
		container.setFieldValue(CorbaConstants.Z_END_NE_STR, handler
				.getValueByName(value.zEndPoint.name,
						CorbaConstants.MANAGED_ELEMENT_STR));
		container.setFieldValue(CorbaConstants.Z_END_PTP_STR, handler
				.getValueByName(value.zEndPoint.name, CorbaConstants.PTP_STR));
		container.setFieldValue(CorbaConstants.A_END_VLAN_STR,
				String.valueOf(value.aEndPoint.vlanID));
		container.setFieldValue(CorbaConstants.A_END_VC_STR,
				String.valueOf(value.aEndPoint.vc));
		container.setFieldValue(CorbaConstants.A_END_TUNEL_STR,
				String.valueOf(value.aEndPoint.tunnel));
		container.setFieldValue(CorbaConstants.Z_END_VLAN_STR,
				String.valueOf(value.zEndPoint.vlanID));
		container.setFieldValue(CorbaConstants.Z_END_VC_STR,
				String.valueOf(value.zEndPoint.vc));
		container.setFieldValue(CorbaConstants.Z_END_TUNEL_STR,
				String.valueOf(value.zEndPoint.tunnel));
		container
				.setFieldValue(
						CorbaConstants.ADDITIONAL_INFO_STR,
	...
	public void printBindingPath(HW_MSTPBindingPath_T mstpBindingPath,
			NameAndStringValue_T[] vctrunkPort)
			throws ProcessingFailureException, SAXException {
		handler.printStructure(getBindingPathParams(mstpBindingPath,
				vctrunkPort));
	}

	public Corba2XMLContainer getBindingPathParams(
			HW_MSTPBindingPath_T mstpBindingPath,
			NameAndStringValue_T[] vctrunkPort)
			throws ProcessingFailureException {
		String direction = null;
		...
		Corba2XMLContainer container = new Corba2XMLContainer(
				Corba2XMLStructure.BINDING_PATH);
		container.setFieldValue(CorbaConstants.VCTRUNK_NE_STR,
				handler.getValueByName(vctrunkPort,
						CorbaConstants.MANAGED_ELEMENT_STR));
